...
+ (NSString *)buildUrl:(NSString*)forecastType :(NSString *)city :(NSString *)unit {
    ...
    if ([forecastType  isEqual: @"forecast/daily"]) {
        ...
        unitsString = @"";
        unit = @"";
        rootUrl = @"https://samples.openweathermap.org/data/2.5/";
    } else {
        unitsString = @"&units=";
        rootUrl = @"https://api.openweathermap.org/data/2.5/";
    }
//    NSString *rootUrl = @"https://api.openweathermap.org/data/2.5/";
    NSString *url = [NSString stringWithFormat:@"%@%@?appid=c2394aa8aa8b414a1638b9cf3376b22a&q=%@%@%@",rootUrl, forecastType, city, unitsString, unit];
